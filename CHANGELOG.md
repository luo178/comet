...
- **Phase-skip enforcement across all guard layers**: Fixed agents being able to jump from `open` straight to `build` (skipping `design`) without any layer catching it. The root cause was that evidence invariants were only enforced in `comet-guard.sh` and in 2 of 5 state transitions, while the hook and rule only checked the *current* `phase` field, never *how* it was reached. Hardened four layers: (1) `comet-state.sh` now enforces evidence on every forward transition — `open-complete` requires non-empty proposal/design/tasks, `design-complete` requires an existing `design_doc`, and `archived` requires `verify_result: pass`, matching the existing `build-complete`/`verify-pass` checks; (2) direct `comet-state set <name> phase <value>` is now blocked (it bypassed all evidence checks), with a `COMET_FORCE_PHASE=1` escape hatch reserved for repairing malformed state; (3) `comet-hook-guard.sh` now blocks source writes when `workflow: full` reaches `build`/`verify` with a null `design_doc`, catching illegal jumps regardless of how the phase was set (misclassified preset, direct phase write, or bare transition); (4) the `comet-phase-guard` rule (Chinese and English) — the only runtime defense on hook-less platforms — adds a "phase-entry self-consistency check" table that requires agents to verify the prerequisite artifacts exist before writing source code, prohibits manual phase jumps, and re-runs the consistency check after context-compression recovery.
